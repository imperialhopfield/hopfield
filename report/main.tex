...
Mental Disorders are a major problem that affects a large number of people in
our society. Understanding the human brain is one of the first steps that can be
taken in an attempt to cure these disorders. Recent research has shown that a mathematical way of analysing these disorders is starting to become feasible. 

Our project is trying to model, on a methaforical level, how an individual will react given various positive or negative memories. We have implemented a mathematical model of neural networks, called the Hopfield model, that can simulate various functions of the brain. Amongst these, two of them are of uttermost importance: learning and recalling information. Using the Hopfield model, we have analysed how easy different memories are recalled, how probable it is for one memory to be recalled, or how some memories can overwrite others. The latter is extemely important for the psychological studies, since it provides a way of curing some mental disorders that have been caused by negative memories. 

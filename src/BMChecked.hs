...
--gimmeRandom = do
--  r <- runRVar (uniform 0 100 :: RVar Int) (getRandom:: m Word64)


module BolzmannMachine where
-- | Base Restricted Bolzmann machine.
...
-- In the case of the Bolzmann Machine the weight matrix establishes the
...
updateWS ws v = do
  ...
  let pos   = NC.toLists $ (f v) `NC.outer` (f h)
  let neg   = NC.toLists $ (f v') `NC.outer` (f h')
  let dws   = combine (-) pos neg
  let ws_f  = combine (+) (list2D ws) dws
  let final = map (map (* lr)) ws_f
